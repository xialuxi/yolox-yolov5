        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(progress_bar(self.dataloader)):
            with torch.no_grad():
                ...
                for si, pred in enumerate(outputs):
                    ...
                    if pred is None:
                        if nl:
                            stats.append((torch.zeros(0, niou, dtype=torch.bool).cuda(), torch.Tensor(), torch.Tensor(), gt_cls))
                        continue
                    ...
                    if nl:
                        ...
                        stats.append((correct, pred[:, 4], pre_cls, gt_cls))
                                     
                    
        if distributed:
            stats = gather(stats, dst=0)
            seens = gather(seen, dst=0)
            new_stats = []
            seen = 0
            for i in range(len(stats)):
                new_stats.extend(stats[i])
                seen = seen + seens[i].sum().cpu()

        if self.local_rank == 0:
            # Compute statistics
            stats = []
            for stat in new_stats:
                correct, conf, pre_cls, gt_cls = stat
                stats.append((correct.cpu(), conf.cpu(), pre_cls.cpu(), gt_cls.cpu()))
